...
La corrente che necessita il motore DC in questione per funzionare va spesso oltre i $100mA$. Per fornire tale corrente al motore non è sufficiente collegare direttamente il *PWM* finale ad esso, bensì è fondamentale amplificare la corrente. Un metodo per farlo consiste nell'aggiungere dei *transistor darlington* (lo schema è mostrato in Fig.\ref{fig:darlington}) presenti nel circuito integrato *ULN2003A*, corrispondente all'integrato numero 6. Ogni canale di questo pacchetto può amplificare la corrente fino a $300mA$, connettendone due in parallelo quindi si possono raggiungere anche i $600mA$, più che sufficienti per il circuito. 
...
```{r gen, fig.cap="Generatore di tensione", out.width = "60%", }
knitr::include_graphics("immagini/generatore.png")
```

In quest'immagine vengono misurate tensione, corrente, e potenza, assorbite dal circuito quando il motore lavora in folle. Dal primo canale, in giallo, è fornita la tensione per le componenti elettroniche del circuito, mentre dal secondo canale, in blu, quella per il motore. La corrente del secondo canale in realtà non è costante, questo probabilmente è dato dal fatto che il *PWM* del freno non ha un *duty-cycle* costante proprio per il metodo utilizzato. Tentare di misurare con il multimetro questa corrente sarebbe vano.
Il massimo valore di corrente osservato, quando viene premuto il pulsante dell'acceleratore, è circa $150mA$, con una potenza corrispondente di circa $750mW$.

I dati misurati con il multimetro del primo generatore risultano molto simili a quelli scritti sul *display* del generatore:
